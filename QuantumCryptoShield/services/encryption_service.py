...
class EncryptionService:
    ...
    def generate_keys(self, tenant_id):
        # Genera una chiave quantistica di 32 bytes (256 bits) per AES-256
        quantum_key = self.qrng_service.generate_qrng_key(length=32)
        if not quantum_key:
            # Usa un generatore di numeri casuali standard come fallback
    ...
    def encrypt(self, tenant_id, message):
        ...
        iv = os.urandom(16)  # Inizializzazione per AES CBC
        ...
        # Restituisci iv + ciphertext codificati in esadecimale
